function Calendar(data) {
    ...
    vam('#main').innerHTML =
        `<div id="calender" class="Wrap_Main">
            <div id="calender-nav" class="Wrap_Nav">
                <h1 class="nav-title">Sổ đầu bài</h1>
            </div>
            <div id="calender-learn" class="Wrap_Content">
                <div id="now">
                    <div id="celender_more">
                        <div id="CreateCelender"><input type="date" value="${formatDate(today)}"></div>
                    </div>
                    <div id="LearnToday">
    ...
    vam('#CreateCelender>input').onchange = () => {
        vam('#LearnToday_list').innerHTML = ''
        CelenderNow(formatDate(vam('#CreateCelender>input').value), data)
    }
    CelenderNow(formatDate(today), data)
